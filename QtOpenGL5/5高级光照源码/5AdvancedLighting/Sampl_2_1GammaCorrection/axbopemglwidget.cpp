...
QVector3D viewInitPos(0.0f,1.0f,4.0f);
...
void AXBOpemglWidget::initializeGL()
{
    ...
    m_DiffuseTex=new
            QOpenGLTexture(QImage(":/images/images/wall.jpg"));

    QImage wall = QImage(":/images/images/wall.jpg").convertToFormat(QImage::Format_RGB888);
    m_gammaDiffuseTex=new QOpenGLTexture(QOpenGLTexture::Target2D);
    glBindTexture(GL_TEXTURE_2D, m_gammaDiffuseTex->textureId());

    glTexImage2D(GL_TEXTURE_2D, 0, GL_SRGB, wall.width(), wall.height(),
                 0, GL_RGB , GL_UNSIGNED_BYTE, wall.bits());
    glGenerateMipmap(GL_TEXTURE_2D);

    m_PlaneMeshGamma=processMesh(planeVertices,6,m_gammaDiffuseTex->textureId());
...
void AXBOpemglWidget::paintGL()
{
    ...
    m_ShaderProgram.setUniformValue("light.diffuse", 0.7f, 0.7f, 0.7f);
    m_ShaderProgram.setUniformValue("light.specular", 1.0f, 1.0f, 1.0f);
    // material properties
    m_ShaderProgram.setUniformValue("material.shininess", 32.0f);
    m_ShaderProgram.setUniformValue("light.pos", 0.0f, 0.0f, 0.0f);
    m_ShaderProgram.setUniformValue("model", model);
    m_ShaderProgram.setUniformValue("gamma", gamma);

    if(gamma)
        m_PlaneMeshGamma->Draw(m_ShaderProgram);
    else

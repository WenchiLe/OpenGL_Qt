...
void AXBOpemglWidget::initializeGL()
{
    ...
    m_ReflectionShaderProgram.addShaderFromSourceFile(QOpenGLShader::Vertex,":/shaders/shaders/reflection.vert");
    m_ReflectionShaderProgram.addShaderFromSourceFile(QOpenGLShader::Fragment,":/shaders/shaders/reflection.frag");
    success=m_ReflectionShaderProgram.link();
    if(!success) qDebug()<<"ERR:"<<m_ReflectionShaderProgram.log();
...
void AXBOpemglWidget::paintGL()
{
    ...
    m_ReflectionShaderProgram.bind();
    m_ReflectionShaderProgram.setUniformValue("projection", projection);
    m_ReflectionShaderProgram.setUniformValue("view", view);
    m_ReflectionShaderProgram.setUniformValue("viewPos",m_camera.Position);
    foreach(auto modelInfo,m_Models){
        ...
        m_ReflectionShaderProgram.setUniformValue("model", model);
        modelInfo.model->Draw(m_ReflectionShaderProgram);
    }
    // draw skybox as last

...
void AXBOpemglWidget::initializeGL()
{
  ...
  //  glEnable(GL_PROGRAM_POINT_SIZE);
    ...
    int uniformBlockIndexShader =
            glGetUniformBlockIndex(m_ShaderProgram.programId(),"Matrices");
    int uniformBlockIndexSkyBoxShader =
            glGetUniformBlockIndex(m_SkyBoxShaderProgram.programId(),"Matrices");

    glUniformBlockBinding(m_ShaderProgram.programId(),
                          uniformBlockIndexShader, 0);
    glUniformBlockBinding(m_SkyBoxShaderProgram.programId(),
 ...
 //   m_CubeMap->setWrapMode(QOpenGLTexture::DirectionR, QOpenGLTexture::ClampToEdge);
    ...
    glBindBufferRange(GL_UNIFORM_BUFFER, 0, uboMatrices, 0, 2 * QMatrix4x4DataSzie);
   // qDebug()<<"sizeof(QMatrix4x4)="<<sizeof(QMatrix4x4)<<"?"<<4*4*4;
...
//    glBindBuffer(GL_UNIFORM_BUFFER, uboMatrices);
    ...
    m_SkyBoxShaderProgram.setUniformValue("skyView", skyboxView);
    // skybox cube
    glBindVertexArray(skyboxVAO);

...
void AXBOpemglWidget::initializeGL()
{
    ...
    m_PostProcessingShaderProgram.addShaderFromSourceFile(QOpenGLShader::Vertex,":/shaders/shaders/shapes.vert");
    m_PostProcessingShaderProgram.addShaderFromSourceFile(QOpenGLShader::Fragment,":/shaders/shaders/light.frag");
    success=m_PostProcessingShaderProgram.link();
    if(!success) qDebug()<<"ERR:"<<m_PostProcessingShaderProgram.log();
...
m_PlaneMesh->Draw(m_ShaderProgram);
        ...
        for(map<float,QVector3D>::reverse_iterator riter=sorted.rbegin();
            ...
            m_PostProcessingShaderProgram.setUniformValue("model", model);
            m_ShaderProgram.setUniformValue("projection", projection);
            m_ShaderProgram.setUniformValue("view", view);
            m_WindowMesh->Draw(m_PostProcessingShaderProgram);
        }

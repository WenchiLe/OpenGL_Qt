...
void AXBOpemglWidget::initializeGL()
{
    ...
    m_NormalShaderProgram.addShaderFromSourceFile(QOpenGLShader::Vertex,":/shaders/shaders/normal.vert");
    m_NormalShaderProgram.addShaderFromSourceFile(QOpenGLShader::Geometry,":/shaders/shaders/normal.geom");
    m_NormalShaderProgram.addShaderFromSourceFile(QOpenGLShader::Fragment,":/shaders/shaders/normal.frag");
    success=m_NormalShaderProgram.link();
    if(!success) qDebug()<<"ERR:"<<m_NormalShaderProgram.log();
  ...
  //  m_PlaneMesh=processMesh(points,4,m_PlaneDiffuseTex->textureId());
}
...
void AXBOpemglWidget::paintGL()
{
    ...
    m_ShaderProgram.setUniformValue("light.ambient", 1.0f, 1.0f, 1.0f);
    m_ShaderProgram.setUniformValue("light.diffuse", 1.0f, 1.0f, 1.0f);
  ...
  //  m_PlaneMesh->Draw(m_ShaderProgram);
    ...
    m_NormalShaderProgram.setUniformValue("projection", projection);
    m_NormalShaderProgram.setUniformValue("view", view);
    m_NormalShaderProgram.release();
    foreach(auto modelInfo,m_Models){
        ...
        m_NormalShaderProgram.bind();
        m_NormalShaderProgram.setUniformValue("model", model);
        modelInfo.model->Draw(m_NormalShaderProgram);
        m_NormalShaderProgram.release();
    }
